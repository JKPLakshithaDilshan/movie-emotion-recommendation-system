...
OUT = Path("IT25100083/IT25100083_linear_svm.ipynb")
...
md(r'''# IT25100083 – Linear Support Vector Machine
...
md('''### EDA 1 — Description word-count histogram''')
...
md(r'''**Interpretation:** Description lengths are right-skewed, with a small number of unusually long summaries. This supports IQR-based outlier treatment and scaling of engineered length features.''')

md('''### EDA 2 — Description word-count boxplot''')
...
plt.show()''')
md(r'''**Interpretation:** Observations beyond the upper whisker are potential length outliers. They are clipped rather than deleted because long descriptions remain legitimate movies and deletion could worsen class imbalance.''')

md('''### EDA 3 — Description length by emotion''')
code(r'''order = df.groupby("emotion")["description_word_count"].median().sort_values().index
...
plt.show()''')
md(r'''**Interpretation:** The emotion groups overlap strongly in description length. Length is useful as a supporting engineered feature but cannot replace semantic TF-IDF features.''')

md('''### EDA 4 — Description word count versus mean rating''')
code(r'''fig, ax = plt.subplots(figsize=(10, 6))
...
md(r'''**Interpretation:** The scatter shows weak separation and substantial overlap, so neither rating nor length alone explains emotion. Combining text, encoded genre, and scaled numeric features is justified.''')
...
md(r'''## 7. Categorical encoding and target encoding — primary specialization
