...
OUT = Path("IT25100078/IT25100078_naive_bayes.ipynb")
...
md(r'''# IT25100078 – Multinomial Naive Bayes

**Student:** Thujikoshan Y.  
**IT number:** IT25100078  
**Project:** Emotion-Aware Movie Recommendation System  
**Primary preprocessing responsibility:** Feature selection  
...
MEMBER_DIR = PROJECT_ROOT / "IT25100078"
...
md('''### EDA 1 — Top-genre frequency chart''')
code(r'''genre_long = (
    df.assign(genre_token=df["genres_text"].str.split())
    .explode("genre_token")
)
genre_long["genre_token"] = genre_long["genre_token"].fillna("unknown_genre")
top_genres = genre_long["genre_token"].value_counts().head(15).sort_values()

fig, ax = plt.subplots(figsize=(10, 7))
top_genres.plot(kind="barh", color="#2563eb", ax=ax)
ax.set_title("Top 15 Genre Frequencies")
ax.set_xlabel("Number of descriptions")
ax.set_ylabel("Genre")
fig.tight_layout()
fig.savefig(EDA_DIR / "01_top_genre_frequency.png", dpi=300, bbox_inches="tight")
plt.show()
display(top_genres.sort_values(ascending=False).to_frame("description_count"))''')
md(r'''**Interpretation:** A few genres occur much more often than others, while rare genres provide limited evidence. Genre is therefore retained as supporting categorical information, but semantic description features remain central to the classifier.''')

md('''### EDA 2 — Genre-count distribution''')
code(r'''fig, ax = plt.subplots(figsize=(10, 6))
sns.histplot(df["genre_count"], discrete=True, color="#14b8a6", ax=ax)
ax.set_title("Number of Genres per Description")
ax.set_xlabel("Genre count")
ax.set_ylabel("Frequency")
fig.tight_layout()
fig.savefig(EDA_DIR / "02_genre_count_distribution.png", dpi=300, bbox_inches="tight")
plt.show()
display(df["genre_count"].value_counts().sort_index().to_frame("description_count"))''')
md(r'''**Interpretation:** Most descriptions have only a small number of genres, while empty lists are represented by `unknown_genre`. The count supplies compact information about genre breadth and is checked for outliers before modelling.''')

md('''### EDA 3 — Genre and emotion heatmap''')
code(r'''major_genres = genre_long["genre_token"].value_counts().head(12).index
major_genre_rows = (
    genre_long.loc[
        genre_long["genre_token"].isin(major_genres)
    ]
    .reset_index(drop=True)
)

genre_emotion = pd.crosstab(
    major_genre_rows["genre_token"],
    major_genre_rows["emotion"],
)

fig, ax = plt.subplots(figsize=(12, 8))
sns.heatmap(genre_emotion, cmap="Blues", annot=True, fmt="d", ax=ax)
ax.set_title("Major Genres by Emotion")
ax.set_xlabel("Emotion")
ax.set_ylabel("Genre")
fig.tight_layout()
fig.savefig(EDA_DIR / "03_genre_emotion_heatmap.png", dpi=300, bbox_inches="tight")
plt.show()''')
md(r'''**Interpretation:** Genre–emotion frequencies are uneven and partly reflect the overall class imbalance. Some genres show different emotional profiles, justifying categorical genre encoding, but no genre uniquely determines the target.''')

md('''### EDA 4 — Mean rating by major genre''')
code(r'''major_genre_ratings = (
    genre_long[genre_long["genre_token"].isin(major_genres)]
    .groupby("genre_token", as_index=False)
    .agg(mean_rating=("mean_rating", "mean"), description_count=("Description", "nunique"))
    .sort_values("mean_rating", ascending=False)
)

fig, ax = plt.subplots(figsize=(11, 6))
sns.barplot(data=major_genre_ratings, x="genre_token", y="mean_rating", color="#7c3aed", ax=ax)
ax.set_title("Mean Rating by Major Genre")
ax.set_xlabel("Genre")
ax.set_ylabel("Mean rating")
ax.tick_params(axis="x", rotation=40)
ax.set_ylim(0, 10)
fig.tight_layout()
fig.savefig(EDA_DIR / "04_mean_rating_by_major_genre.png", dpi=300, bbox_inches="tight")
plt.show()
display(major_genre_ratings)''')
md(r'''**Interpretation:** Average ratings vary across major genres, but the differences are not large enough to act as a standalone predictor. Rating and genre are therefore supporting features alongside TF-IDF description terms.''')
...
`genres_text` is encoded with `OneHotEncoder(handle_unknown='ignore')`, allowing unseen test categories without failure. `LabelEncoder` is demonstrated for the target, although scikit-learn Multinomial Naive Bayes can train directly on string labels.''')
...
StandardScaler is demonstrated below to show standardization. In the final pipeline, MinMaxScaler is used after median imputation and IQR clipping so numeric values remain non-negative. Non-negative inputs are required by both chi-squared selection and Multinomial Naive Bayes. All scaling parameters are fitted only on training folds.''')
...
- Classifier: Multinomial Naive Bayes, which is efficient for high-dimensional, non-negative text-like features.
...
md(r'''## 11. Feature selection — primary specialization

Chi-squared feature selection ranks non-negative features by their association with the emotion target. The tuning process compares 500, 1,000, and 1,500 selected features to determine whether stronger reduction or broader vocabulary generalizes better. Selection is fitted inside every cross-validation fold to prevent leakage.''')

md(r'''## 12. Dimensionality reduction demonstration

TruncatedSVD is appropriate for demonstrating dimensionality reduction on sparse TF-IDF data and is fitted using training descriptions only. SVD components can contain negative values, so they cannot be supplied directly to Multinomial Naive Bayes, which requires non-negative inputs. For this reason, SVD is demonstrated separately while the final model uses non-negative chi-squared-selected features.''')
...
The search compares four additive-smoothing (`alpha`) values, learned versus uniform class priors, three feature-selection sizes, and unigram versus unigram-bigram TF-IDF. This produces 48 model varieties and 240 total fits under five-fold validation. Macro F1 is optimized because the classes are imbalanced.''')
...
model_comparison.to_csv(OUTPUT_DIR / "naive_bayes_model_comparison.csv", index=False)''')
...
metrics_df.to_csv(OUTPUT_DIR / "naive_bayes_metrics.csv")

report = classification_report(y_test, y_test_pred, output_dict=True, zero_division=0)
report_df = pd.DataFrame(report).T
display(report_df.round(4))
report_df.to_csv(OUTPUT_DIR / "naive_bayes_classification_report.csv")''')
...
fig.tight_layout()
fig.savefig(MODEL_FIG_DIR / "naive_bayes_confusion_matrix.png", dpi=300, bbox_inches="tight")
plt.show()''')
...
cv_summary.to_csv(OUTPUT_DIR / "naive_bayes_cross_validation.csv")''')
...
predictions.to_csv(OUTPUT_DIR / "naive_bayes_test_predictions.csv", index=False)
model_path = OUTPUT_DIR / "IT25100078_best_naive_bayes.pkl"
...
with open(OUTPUT_DIR / "naive_bayes_summary.json", "w", encoding="utf-8") as file:
...
**Model selection:** The final Multinomial Naive Bayes variety is selected using five-fold cross-validated macro F1, not test-set performance. This protects the test set and gives equal importance to minority classes.
...
- Compare Complement Naive Bayes, calibrated linear SVM, and transformer embeddings.
- Conduct fairness/error analysis across genres.
- Explain predictions using class-conditional feature log probabilities.

## 20. Individual reflection

This implementation demonstrates that reliable evaluation depends on data quality, leakage-safe preprocessing, stratification, and metrics suited to imbalance. Feature selection reduces noise and computation while retaining the terms most associated with emotion. Multinomial Naive Bayes is an efficient baseline for non-negative TF-IDF features, although its conditional-independence assumption limits its ability to represent complex language relationships.
